...
const ACTION_START_PATTERN = /^(打开|关掉|关闭|拿出|放到|放进|走到|点击|新建|复制|粘贴|输入|写下|读完|标记|找到|翻到|备好|换上|穿上|穿鞋|下楼|出门|前往|走去|到达|挑选|购买|付款|拿走|带回|回家|返回|坐下|保存|提交|发送|拍照|清理|归位|按下|选择|启动|运行|朗读|检查|记录|退出|只保留)/;
const VAGUE_STEP_PATTERN = /^(写|准备|整理|学习|研究|思考|处理|推进|继续|完成|做|弄|搞|优化|修改|看|阅读)(?!下|完|到|开|出|好)/;

const TASK_CONTEXTS = [
  {
    test: (t) => /早餐|早饭/.test(t) && /买|下楼|便利店|超市|商店|出去|外出/.test(t),
    simple: false,
    steps: () => [
      { label: `穿鞋下楼`, minutes: 2 },
      { label: `前往便利店`, minutes: 3 },
      { label: `挑选并购买早餐`, minutes: 4 },
      { label: `回家`, minutes: 3 },
    ],
  },
  {
    test: (t) => /咖啡|奶茶|饮料/.test(t) && /买|下楼|便利店|店|出去|外出/.test(t),
    simple: false,
    steps: (title) => {
      const item = /奶茶/.test(title) ? "奶茶" : /饮料/.test(title) ? "饮料" : "咖啡";
      return [
        { label: `穿鞋下楼`, minutes: 2 },
        { label: `前往附近商店`, minutes: 3 },
        { label: `挑选并购买${item}`, minutes: 4 },
        { label: `带${item}回家`, minutes: 3 },
      ];
    },
  },
  {
    ...
    simple: false,
    steps: (title) => {
      const itemMatch = title.match(/买(.{1,8})/);
      const item = itemMatch ? itemMatch[1].replace(/[，。,.!！?？].*$/, "") : "东西";
      const core = /取/.test(title) ? "取到物品" : /寄|送/.test(title) ? "寄送物品" : `挑选并购买${item}`;
      return [
        { label: `穿鞋出门`, minutes: 2 },
        { label: `前往目的地`, minutes: 3 },
        { label: core, minutes: 4 },
...
const SYSTEM_PROMPT = `你是 FocusBoost 的 ADHD 友好任务拆解 AI。用户会用中文描述一个待办或一整句话（可能来自语音转写）。
...
function summarizeTitle(text) {
  ...
  // 规范化生活任务标题：保留核心结果，不把路径写进主线标题
  t = t.replace(/下去买/g, "下楼买");
  if (/早餐|早饭/.test(t) && /买|购买|下楼|便利店|超市|商店/.test(t)) return "买早餐";
  if (/咖啡/.test(t) && /买|购买|下楼|便利店|咖啡店|商店/.test(t)) return "买咖啡";
  if (/奶茶/.test(t) && /买|购买|下楼|便利店|奶茶店|商店/.test(t)) return "买奶茶";
...
function actionizeStepLabel(label) {
  ...
  if (/早餐|早饭/.test(raw) && /买|购买|挑选/.test(raw)) return "挑选并购买早餐";
  if (/便利店|超市|商店|目的地/.test(raw) && /前往|去|走到/.test(raw)) return raw.startsWith("前往") ? raw : "前往便利店";
  if (/回家|返回/.test(raw)) return raw.includes("回家") ? raw : "回家";
  if (/下楼|出门|穿鞋/.test(raw)) return raw.includes("穿鞋") ? raw : "穿鞋下楼";
